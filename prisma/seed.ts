...
'''`.trim()
  ...
  await prisma.prompt.create({
    data: {
      title: 'Quiz sobre o vídeo do YoutTube',
      template: `Seu papel é gerar um quiz sobre o vídeo do YouTube.

Abaixo você receberá uma transcrição desse vídeo, use essa transcrição para gerar o quiz.
Abaixo você também receberá uma lista de perguntas, use-as como referência para criar o quiz.

O quiz deve ter no máximo 10 perguntas
O quiz deve ser tentar abordar o máximo de pontos diferentes sobre o vídeo, com intuito de estimular o estudo para uma prova.

Retorne apenas conforme o formato a seguir:
'''
- (Conteúdo da pergunta) (número da pergunta):
        a) Opção a
        b) Opção b
        c) Opção c
        d) Opção d
- Repita o processo para mais perguntas
'''
